...
views.byKeyword = {
  map: function(doc) {
    Array.isArray = Array.isArray || function isArray(a) {
      return a instanceof Array || Object.prototype.toString.call(a) === "[object Array]" || (typeof a === "object" && typeof a.length === "number")
    }
    Array.prototype.forEach = Array.prototype.forEach || function forEach(fn) {
      for (var i = 0, l = this.length; i < l; i++) {
        if (this.hasOwnProperty(i)) {
...
views.needBuild = {
  map: function(doc) {
    ...
    if (!d) return
    if (!d.scripts) return
    var inst = d.scripts.install || d.scripts.preinstall || d.scripts.postinstall
    if (!inst) return
    //emit(d.name + "@" + d.version, d.dist.bin || {})
...
views.orphanAttachments = {
  map: function(doc) {
    ...
    if (orphans.length) emit(doc._id, {
      size: size,
      orphans: orphans
    })
  }
...
views.browseAuthors = views.npmTop = {
  map: function(doc) {
    Array.prototype.forEach = Array.prototype.forEach || function forEach(fn) {
      for (var i = 0, l = this.length; i < l; i++) {
        if (this.hasOwnProperty(i)) {
    ...
    var readme = doc.readme || l.readme || ''
    doc.maintainers.forEach(function(m) {
      emit([m.name, doc._id, desc, t, v, readme], 1)
    })
  },
  reduce: "_sum"
}
...
views.analytics = {
  map: function(doc) {
    ...
    for (var i in doc.time) {
      ...
      var d = new Date(parse(t))
      if (!d.getTime()) return
      var type = i === 'modified' ? 'latest' : i === 'created' ? 'created' : 'update'
      emit([type,
        d.getUTCFullYear(),
        d.getUTCMonth() + 1,
        d.getUTCDate(),
